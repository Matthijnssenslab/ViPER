    ...
    # Assembly
        -m | --min-length)
        	if [[ "$2" =~ ^[0-9]+$ ]]; then
        		...
        		>&2 printf '\n%s\n\n' "[ERROR]: Give a minimum length for assembled contigs."
        		exit 1
        	else
...
if [[ $triple -eq 1 ]]; then
	...
	#viper-per-sample-cluster.py -i "$sample"_"$minlength".contigs.fasta -d "$checkv_db" -o clustering/"$sample"_"$minlength" -t "$threads" \
