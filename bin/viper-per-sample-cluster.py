...
def aniclust(
    fasta, anicalc_dict, minlength=0, min_qcov=0, min_tcov=85, min_ani=95  # outname,
):
    ...
    for i in anicalc_dict:
        if anicalc_dict[i]["qname"] == anicalc_dict[i]["tname"]:
            ...
            anicalc_dict[i]["qname"] not in edges
            or anicalc_dict[i]["tname"] not in edges
        ):
            continue
        elif (
            float(anicalc_dict[i]["qcov"]) < min_qcov
            or float(anicalc_dict[i]["tcov"]) < min_tcov
            or float(anicalc_dict[i]["pid"]) < min_ani
        ):
            continue
...
def clustering(fasta, output, threads, pid, cov):
    ...
    makedb = NcbimakeblastdbCommandline(
        dbtype="nucl", input_file=fasta, out="tmp_clustering/blastdb/" + output + "_db"
    )
    ...
    keep = aniclust(
        fasta, anicalc_dict, min_ani=pid, min_tcov=cov  # "tmp_clustering/"+output,
    )
